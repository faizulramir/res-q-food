...
export class UploadImgPage {
  ...
  async imageCropped(event: ImageCroppedEvent) {
    ...
    const img = await this.api.updatePic({ pic: JSON.stringify(this.croppedImage), id: this.user.id})
    const loading = await this.loadingCtrl.create();
    await loading.present();
    if (img) {
      this.loadingCtrl.dismiss();
      this.presentToast(img.msg)
      ...
      this.router.navigate(['index/tabs/profile'], navigationExtras)
    }
  }

  goBack() {
    this.router.navigate(['index/tabs/profile'])
  }
